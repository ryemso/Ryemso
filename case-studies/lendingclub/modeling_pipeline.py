...
This file keeps the main analytical choices found in the Drive notebook while
removing Colab-specific paths and notebook-only visualization code.

Raw data is intentionally not included.
"""
...
def split_and_scale(df: pd.DataFrame):
    """Use the original stratified split and train-only scaling approach."""
...
def resample(X_train, y_train, method: str = "smote"):
    """Compare the two principal resampling approaches used in the notebook."""
    if method == "smote":
...
def run_comparison(resampling: str = "smote") -> pd.DataFrame:
    """End-to-end comparison using the cleaned portfolio pipeline."""
