...
이 문서는 Google Drive의 원본 Colab notebook `9_Project_code.ipynb`를 기준으로 정리했습니다.
...
한 가지 지표만으로 모델을 고르기보다 Recall과 F1, ROC AUC의 trade-off를 함께 비교한 프로젝트입니다.

원본 notebook의 Logistic Regression RandomizedSearch 결과에는:
...
[modeling_pipeline.py](./modeling_pipeline.py)는 원본 Colab notebook의 분석 흐름을 면접/코드리뷰용으로 다시 정리한 파일입니다.
...
원본 notebook 전체를 그대로 복사한 파일이 아니라 **검토 가능한 형태로 재구성한 버전**입니다.

## Data

LendingClub 원본 데이터는 저장소에 포함하지 않습니다.
