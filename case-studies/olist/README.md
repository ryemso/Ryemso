...
브라질 전자상거래 플랫폼 Olist의 2016–2018 공개 데이터를 이용해 **Seller 확보와 고객 유지, 배송 운영 문제**를 분석한 팀 프로젝트입니다.

이 문서는 Google Drive에 보관된 발표자료 `olist_Analysis.pdf`를 기준으로 정리했습니다.

## Business Question

프로젝트의 큰 질문은 다음과 같았습니다.

- B2B: 어떤 Seller를 더 확보해야 marketplace 성장을 만들 수 있는가?
- B2C: 신규 고객을 충성 고객으로 전환하려면 무엇을 개선해야 하는가?
- Operations: 배송 지연 문제를 어디에서 우선 해결해야 하는가?

## Data

발표자료 기준 Olist 공개 데이터는 총 9개 CSV로 구성되어 있습니다.
...
발표자료에서는 분석 목적에 따라 `order_id.nunique()`를 사용하고 Payment merge를 필요한 경우에만 적용하는 방식으로 대응했습니다.

### 2. Missing / abnormal values
...
주요 카테고리의 seller 집중도를 HHI로 확인했으며, 발표자료에서는 대부분의 주요 카테고리가 **HHI < 0.15**로 나타났고 이를 신규 seller 모집이 가능한 경쟁 구조로 해석했습니다.
...
배송기간과 Review Score의 관계를 확인한 결과 발표자료에는:
...
발표자료에는 세 가지 관점의 대시보드가 포함되어 있습니다.
...
현재 연결된 Google Drive에서 확인된 프로젝트 원본은 발표자료 PDF입니다.  
분석 코드 원본은 확인되지 않아, 이 문서는 발표자료가 실제로 지원하는 내용만 정리했습니다.
